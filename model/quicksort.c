...
// struct array to store the PIVOT INDEXES VALUES
struct pArray
{
    int arr[2];
};
...
struct pArray partition(int array[], int leftmostIndex, int rightmostIndex)
{
    ...
    int i = -1;                        // index for the leftmost elements equal to pivot
    int low = 0;                       // index to track the elements less than pivot
    int high = rightmostIndex;         // index to track the elements greater than pivot
...
void quickSort(int array[], int leftmostIndex, int rightmostIndex)
{
    ...
    {
        struct pArray x = partition(array, leftmostIndex, rightmostIndex); // to get pivot indexes
